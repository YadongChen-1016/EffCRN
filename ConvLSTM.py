...
class ConvLSTMCell(nn.Module):
    ...
    def __init__(self, img_size, input_dim, hidden_dim, kernel_size,
                 cnn_dropout, rnn_dropout, padding=None, bias=True, peephole=False,
                 layer_norm=False):
        """
        ...
        if not padding:
            self.out_height = self.input_shape
        else:
            self.out_height = int((self.input_shape[0] - self.kernel_size + 2 * self.paddin) / self.stride + 1)
        ...
        if self.peephole is True:
            self.weight_ci = HadamardProduct((1, self.hidden_dim, self.out_height))
            self.weight_cf = HadamardProduct((1, self.hidden_dim, self.out_height))
            self.weight_co = HadamardProduct((1, self.hidden_dim, self.out_height))
            self.layer_norm_ci = nn.LayerNorm([self.hidden_dim, self.out_height])
            self.layer_norm_cf = nn.LayerNorm([self.hidden_dim, self.out_height])
            self.layer_norm_co = nn.LayerNorm([self.hidden_dim, self.out_height])

        self.cnn_dropout = nn.Dropout(cnn_dropout)
        self.rnn_dropout = nn.Dropout(rnn_dropout)

        self.layer_norm_x = nn.LayerNorm([4 * self.hidden_dim, self.out_height])
        self.layer_norm_h = nn.LayerNorm([4 * self.hidden_dim, self.out_height])
        self.layer_norm_cnext = nn.LayerNorm([self.hidden_dim, self.out_height])
    ...
    def forward(self, input_tensor, cur_state):
        ...
        if self.padding == 0:
            pad_x = self.kernel_size - 1
            x = torch.nn.functional.pad(x, (0, pad_x), "constant", 0)
        ...
        if self.padding == 0:
            ...
            h = torch.nn.functional.pad(h, (0, pad_h), "constant", 0)
    ...
    def init_hidden(self, batch_size):
        ...
        return (torch.zeros(batch_size, self.hidden_dim, height, device=self.input_conv.weight.device),
                torch.zeros(batch_size, self.hidden_dim, height, device=self.input_conv.weight.device))
...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, img_size, input_dim, hidden_dim, kernel_size,
                 ...
                 batch_first=False, padding=None, bias=True, peephole=False,
                 layer_norm=False,
