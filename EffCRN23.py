...
if __name__ == '__main__':
    ...
    model = EffCRN23(in_channels=2, out_channels=2, F=27, N=4)
    TO_SAMPLES = TorchOLA(frame_shift=hop_length)
    TO_FRAME = TorchSignalToFrames(frame_size=n_fft, frame_shift=hop_length)
    h_zeros_1 = torch.zeros(1, 27, 9, dtype=x.dtype, device=x.device)
    c_zeros_1 = torch.zeros(1, 27, 9, dtype=x.dtype, device=x.device)
    h_zeros_2 = torch.zeros(1, 9, 27 * 9, dtype=x.dtype, device=x.device)
